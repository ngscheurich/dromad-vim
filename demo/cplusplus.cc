...
namespace functional_list // basic building block for template meta programming
{
  ...
  struct integer
  {
    ...
    friend std::ostream &operator<<(std::ostream &os, integer<i> const)
    {
      os << integer<i>::value;
      return os;
    }
...
template <typename L, int current, int moder>
constexpr auto construct_loop()
{
